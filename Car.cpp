...
Car::Car(const Car& C)
{
    ...
    this->brand = C.brand;
    this->model = C.model;
    this->deliveryCities = C.deliveryCities;
    this->deliveryTimes = C.deliveryTimes;
    this->cargoVolume = C.cargoVolume;

    std::cout << "Вызван конструктор копирования класса Car" << "\n";
}
